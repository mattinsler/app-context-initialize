  ...
  return function(context) {
    ...
    initializerConfigs.forEach(function(initializer) {
      if (initializer.config) {
        if (Array.isArray(initializer.config)) {
          initializer.arguments = initializer.config.map(function(c) {
            return resolveConfig(context, c);
          });
        } else {
          initializer.arguments = [resolveConfig(context, initializer.config)];
        }
      } else {
    ...
    return installDependencies(dependencies).then(function(modules) {
      ...
      return initializerConfigs.reduce(function(lastPromise, initializer) {
        return lastPromise.then(function() {
          return initializer.method.apply(initializer, initializer.arguments)(context);
        });
      }, q());
    });
  };
...
module.exports.context = function(appContext, config) {
  if (config.setup) {
    if (!Array.isArray(config.setup)) { config.setup = [config.setup]; }
    appContext.use(1, module.exports.apply(null, config.setup));
  }
  if (config.configured) {
    if (!Array.isArray(config.configured)) { config.configured = [config.configured]; }
    appContext.use(3, module.exports.apply(null, config.configured));
  }
  if (config.connected) {
    if (!Array.isArray(config.connected)) { config.connected = [config.connected]; }
    appContext.use(5, module.exports.apply(null, config.connected));
  }
  if (config.initialized) {
    if (!Array.isArray(config.initialized)) { config.initialized = [config.initialized]; }
    appContext.use(7, module.exports.apply(null, config.initialized));
  }
  if (config.running) {
    if (!Array.isArray(config.running)) { config.running = [config.running]; }
    appContext.use(9, module.exports.apply(null, config.running));
  }
};
